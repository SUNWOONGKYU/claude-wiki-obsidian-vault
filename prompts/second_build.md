...
>
> **`__unmapped__` 처리 (v0.3 신규)**: 변수 값이 `__unmapped__`인 폴더는 점검·교정 대상에서 제외.
> 예: `folder_wiki: __unmapped__`이면 wiki 폴더 안 노트 5규칙 위반 점검 안 함.
>
> **vault_role 처리 (v0.3 신규)**: `vault_role: archive`이면 2차 빌드 자체 차단 (archive는 변경 안 함).
